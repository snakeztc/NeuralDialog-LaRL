...
from latent_dialog.utils import LONG, FLOAT, Pack
from latent_dialog.corpora import USR, SYS, BOS, BOD, EOS, STOP_TOKENS, SEL
...
class LatentAgent(Agent):
    ...
    def write(self, max_words=None, stop_tokens=STOP_TOKENS):
        ...
        inpt = self.corpus.sent2id([SYS])
        inpt_var = self.model.np2var(np.array(inpt), LONG).view(1, len(inpt))
        dec_init_s, logprob_z = self.model.z2dec(self.lang_h, requires_grad=False)

        _, outs = self.model.decoder.write(inpt_var, dec_init_s, self.lang_os, max_words, self.model.vocab, stop_tokens, self.goal_h)
        if outs[-1] == self.corpus.sent2id([SEL])[-1]:
            eos_patch = self.corpus.sent2id([EOS])
            outs += eos_patch
        inpt += outs
...
class LatentRlAgent(LatentAgent):
    ...
    def write(self, max_words=None, stop_tokens=STOP_TOKENS):
        ...
        if self.use_latent_rl:
            ...
            dec_init_s, logprob_z = self.model.z2dec(self.lang_h, requires_grad=True)
...
class LstmAgent(object):
    ...
    def feed_goal(self, raw_goal):
        self.raw_goal = raw_goal
        processed_goal = self.corpus._process_goal(self.raw_goal)
        goal_id = self.corpus._goal2id(processed_goal)
        self.vec_goals_list = []
        for i, d in enumerate(self.corpus.domains):
            self.vec_goals_list.append(np.expand_dims(np.array(goal_id[d]), axis=0))
            # 7*(1, goal_len), goal_len varies among different domains
        self.dlg_history = [] # len = max_dlg_len

    def bod_init(self):
        self.read([BOS, BOD, EOS], require_bos=False)

    def read(self, inpt, require_bos=True):
        inpt = [BOS] + inpt if require_bos else inpt
        self.dlg_history.append(inpt)

    def write(self, max_words=None, stop_tokens=STOP_TOKENS):
        def prepare_input(dlg_history):
            dlg_utt_lens = [len(turn) for turn in dlg_history]
            max_utt_len = max(dlg_utt_lens)
            dlg_history_id = [self.corpus.pad_to(max_utt_len, self.corpus._sent2id(turn), do_pad=True)
                              for turn in dlg_history]
            vec_ctx_lens = np.array([len(dlg_history_id)]) # (1, )
            vec_ctx_utts = np.expand_dims(np.array(dlg_history_id), axis=0) # (1, ctx_len, max_utt_len)
            return Pack(context_lens=vec_ctx_lens, 
                        contexts=vec_ctx_utts, 
                        goals_list=self.vec_goals_list)

        max_words = self.args.max_words if max_words is None else max_words
        inpt = prepare_input(self.dlg_history)
        _, outs = self.model.forward_rl(inpt, max_words)
        out_words = self.corpus.id2sent(outs)
        self.read(out_words)
        return out_words
...
class RlAgent(object):
    ...
    def write(self, max_words=None, stop_tokens=STOP_TOKENS):
        ...
        max_words = self.args.max_words if max_words is None else max_words
        inpt = prepare_input(self.dlg_history)
        logprobs, outs = self.model.forward_rl(inpt, max_words)
        self.logprobs.extend(logprobs)
        out_words = self.corpus.id2sent(outs)
        self.read(out_words)
        return out_words
    ...
    def update(self, success):
        self.all_rewards.append(success)
        # standardize the reward
        r = (success - np.mean(self.all_rewards)) / max(1e-4, np.std(self.all_rewards))
        # compute accumulated discounted reward
        g = self.model.np2var(np.array([r]), FLOAT).view(1, 1)
        rewards = []
        for _ in self.logprobs:
